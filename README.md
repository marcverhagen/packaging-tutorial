...
I have found that the current example will still work without requiring "wheel" and without including the third line, but this may be because of the simplicity of the example. The packages listed here will *only* be needed during the build (so your packaging environment need the pip command listed above), but they are not needed by the installled packages.
...
You can make these archives available to the world and anyone can then do their own local pip install.
